...
중앙 저장소의 최초 `main`에는 dependency manifest가 없어 dependency graph 비교
snapshot이 존재하지 않습니다. 따라서 최초 commit
`c60073dda1b5957cbb6dde55067a083cf9a155ba`를 base로 하는 PR에만 명시적 bootstrap
예외를 적용합니다. 이후 모든 base SHA에서는 GitHub dependency review가 반드시
실행되며, 이 조건은 이동 가능한 브랜치명이 아닌 불변 commit SHA를 비교합니다.
